...
getListing(mls) {
	fetch('http://jasonseck.com:1337/single_listing?key=jasonseck&listing_type=listings_multi_family&mls=' + mls)
		.then(response => {
			if (!response.ok) {
				throw Error("Network request failed")
			}
	...
	render() {
		...
		return(
			...
			<p>multi famliy listingview{this.props.match.params.number}</p>
			<p>{description}</p>
			<p>{price}</p>
			<p>{area}</p>
